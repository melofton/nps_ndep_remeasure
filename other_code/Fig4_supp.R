...
mean_growth <- left_join(df, spp_df, by = "common_name") %>%
  ...
  group_by(species, common_name) %>%
  summarize(mean_log_growth = mean(log_AG_carbon_pYear, na.rm = TRUE)) %>%
  select(species, mean_log_growth) %>%
  ungroup() %>%
  arrange(species) %>%
  ...
  mutate(facet_labels = paste0("atop(italic(",labels,"~",genus,"~",spp,"), mean~log~growth~rate:",round(mean_log_growth,3),")"))
...
plot_data <- left_join(final_pred, mean_growth, by = "species") 
  ...
  p <- ggplot(data = plot_data)+
    ...
    labs(color = "", fill = "", x = expression(paste("change in growth log(kg C ", y^-1," ",ind^-1,")")))+
    ggtitle(expression(paste("Marginal effect of N deposition increase (per kg N ", ha^-1," ",y^-1,")")))
  
  ggsave(p,filename = "./visualizations/final_figures/Figure4_supp.png",
         device = "png", height = 6.5, width = 9, units = "in")
