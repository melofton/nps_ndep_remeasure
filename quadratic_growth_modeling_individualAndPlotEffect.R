...
run_model <- function(k, df, sim){
  ...
  p4 ~ dnorm(0,0.001)#; NEED TO VECTORIZE THIS TO N=3 FOR DIFFERENT STATUSES
  p5 ~ dnorm(0,0.001) # ; ALSO VECTORIZE THIS
  ...
  for(t in 1:n_measures){
    ...
    tree_growth_mean[t] <-  ((tree_effect[tree_index[t]] + ndep[t]*p4 + ndep[t]^2 * p5) * tree_agb_latent[t] ^ p2) 
        * exp(-ba_gt[t]*p3)  
  ...
  }  ", fill=T,file=paste0(sim,"-growthModel-",tree_species,".txt"))
  ...
  arrow::write_parquet(mcmc, sink = paste0(sim, "-",tree_species, "-mcmc.parquet"))
  ...
  ggsave(filename = paste0(sim, "-",tree_species,"-mcmc.pdf"), plot = p, device = "pdf")
  ...
  ggsave(filename = paste0(sim, "-", tree_species,"-pairs.pdf"), plot = p, device = "pdf", height = 12, width = 12)
