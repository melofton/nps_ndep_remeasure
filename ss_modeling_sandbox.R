...
run_model <- function(k, df, sim){
  ...
  # add n_status (updown variable to data)
  ...
  p4[1] ~ dunif(min_ndep*0.5, max_ndep*2); NEED TO VECTORIZE THIS TO N=3 FOR DIFFERENT STATUSES
  p4[2] ~ dunif(min_ndep*0.5, max_ndep*2); 
  p4[3] ~ dunif(min_ndep*0.5, max_ndep*2); 
  p5[1] ~ dunif(0.3, 20)  ; ALSO VECTORIZE THIS
  p5[2] ~ dunif(0.3, 20)
  p5[3] ~ dunif(0.3, 20)
  ...
  for(n in 1:ntrees){
    ...
    tree_effect[n] ~ dnorm(plot_effect[plot[n]], tau_plot)
