...
run_model <- function(k, df, sim){
  ...
  noxidep_prop_baseline <- df1$Dep_NoxipropBaseline
  nreddep_prop_baseline <- df1$Dep_NredpropBaseline
  n_measures <- nrow(df1)
  ...
  ssData   <- list(tree_agb_obs = start_measures,
                   ...
                   noxidep_prop_baseline = noxidep_prop_baseline,
                   nreddep_prop_baseline = nreddep_prop_baseline,
  ...
  for(t in 1:n_measures){
    ...
    tree_growth_mean[t] <-  ((tree_effect[tree_index[t]] + noxidep_delta[t]*p4 + nreddep_delta[t]*p5 + sdep_delta[t]*p6 + mat_delta[t]*p7 + map_delta_dm[t]*p8 + noxidep_prop_baseline[t]*p9 + nreddep_prop_baseline[t]*p10) 
    * tree_agb_obs[t] ^ p2) 
        * exp(-ba_gt[t]*p3)  
