...
run_model <- function(k, df, sim){
  ...
  ndep_prop_baseline <- df1$Dep_NpropBaseline
  sdep_baseline <- df1$Dep_Sbaseline
  mat_baseline <- df1$MAT_baseline
  map_baseline <- df1$MAP_baseline_dm
  n_measures <- nrow(df1)
  ...
  ssData   <- list(tree_agb_obs = start_measures,
                   ...
                   ndep_prop_baseline = ndep_prop_baseline,
                   sdep_baseline = sdep_baseline,
                   mat_baseline = mat_baseline,
  ...
  for(t in 1:n_measures){
    ...
    tree_growth_mean[t] <-  ((tree_effect[tree_index[t]] + ndep_delta[t]*p5 + sdep_delta[t]*p6 + mat_delta[t]*p7 + map_delta_dm[t]*p8 + ndep_prop_baseline[t]*p9 + ndep_delta[t]*sdep_baseline[t]*p10 + ndep_delta[t]*mat_baseline[t]*p11 + ndep_delta[t]*map_baseline[t]*p12) 
    * tree_agb_obs[t] ^ p2) 
        * exp(-ba_gt[t]*p3)  
