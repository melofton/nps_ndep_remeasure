...
run_model <- function(k, df, sim){
  ...
  for(t in 1:n_measures){
    ...
    tree_growth_mean[t] <-  ((tree_effect[tree_index[t]] + noxidep_historic[t]*p4 + nreddep_historic[t]*p5 + sdep_historic[t]*p6 + noxidep_diff[t]*p7 + nreddep_diff[t]*p8 + sdep_diff[t]*p9) 
    * tree_agb_obs[t] ^ p2) 
        * exp(-ba_gt[t]*p3)  
