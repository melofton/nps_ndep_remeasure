...
run_model <- function(k, df, sim){
  ...
  focal_data <- df |> 
    ...
    dplyr::filter(!is.na(subp_BA_GT_m1) & !is.na(Dep_Noxi) & !is.na(Dep_Nred)) |>
    ungroup() 
  
  baseline_dep <- focal_data %>%
    select(common_name, tree_ID, interval_no, Dep_Noxi15, Dep_Noxi, Dep_Nred15, Dep_Nred, Dep_S15, Dep_S, MAT, MAP, date_m2, date_m1) %>%
    filter(interval_no == 1) %>%
    mutate(dt = as.numeric(date_m2 - date_m1)/365) %>%
    mutate(total_years = 15 + dt) %>%
    mutate(Dep_Noxibaseline = (Dep_Noxi15*total_years - Dep_Noxi*dt) / 15,
           Dep_Nredbaseline = (Dep_Nred15*total_years - Dep_Nred*dt) / 15,
           Dep_Sbaseline = (Dep_S15*total_years - Dep_S*dt) / 15) %>%
    select(tree_ID, Dep_Noxibaseline, Dep_Nredbaseline, Dep_Sbaseline)
  ...
  focal_data2 <- left_join(focal_data, baseline_dep, by = "tree_ID") %>%
    mutate(Dep_Noxidelta = Dep_Noxi - Dep_Noxibaseline,
           ...
           MAP_dm = MAP * 0.01) %>%
    filter(!is.na(Dep_Noxibaseline) & !is.na(Dep_Noxidelta) & !is.na(Dep_Nredbaseline) & !is.na(Dep_Nreddelta) & !is.na(Dep_Sbaseline) & !is.na(Dep_Sdelta))
  ...
  df1 <- focal_data2 |> 
    mutate(dt = as.numeric(date_m2 - date_m1)/365) |> 
    select(AG_carbon_pYear, AG_carbon_m1, AG_carbon_m2, tree_ID, plot_ID, dt, Dep_Noxi, Dep_Noxibaseline, Dep_Noxidelta, Dep_Nred, Dep_Nredbaseline, Dep_Nreddelta, subp_BA_GT_m1, MAT, MAP_dm, Dep_S, Dep_Sbaseline, Dep_Sdelta) |>
    dplyr::filter(tree_ID %in% live_tree_ids) |>
  ...
  noxidep_baseline <- df1$Dep_Noxibaseline
  noxidep_delta <- df1$Dep_Noxidelta
  nreddep_baseline <- df1$Dep_Nredbaseline
  ...
  for(t in 1:n_measures){
    ...
    tree_growth_mean[t] <-  ((tree_effect[tree_index[t]] + noxidep_delta[t]*p5 + nreddep_delta[t]*p6 + sdep_delta[t]*p7 + mat[t]*p8 + map_dm[t]*p9) 
    * tree_agb_obs[t] ^ p2) 
        * exp(-ba_gt[t]*p3)  
