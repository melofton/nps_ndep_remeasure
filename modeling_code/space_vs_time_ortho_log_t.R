...
run_model <- function(k, df, sim){
  ...
  procErr ~ dunif(0.0001,10) # this should be gamma
  ...
  for(t in 1:n_measures){
    ...
    log_tree_growth_obs[t] ~ dnorm(log_tree_growth_mean[t], procErr*lambda) # consider t distribution
    # t distribution has one parameter, degrees of freedom, which is nu
    # could model this as normal w/ lambda paramter modeled as gamma
    # gamma has two parameters determined by nu --> this no longer makes sense to me 01 oct 25
