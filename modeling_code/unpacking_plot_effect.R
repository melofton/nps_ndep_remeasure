...
run_model <- function(k, df, sim){
  ...
  ozone_delta <- df1$Ozone_avg_delta
  n_measures <- nrow(df1)
  ...
  ssData   <- list(tree_agb_obs = start_measures,
                   ...
                   noxidep_baseline = noxidep_baseline,
                   nreddep_baseline = nreddep_baseline,
