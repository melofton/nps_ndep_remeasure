...
run_model <- function(k, df, sim){
  ...
  p5 ~ dnorm(0,0.001)#; NEED TO VECTORIZE THIS TO N=3 FOR DIFFERENT STATUSES
  ...
  for(t in 1:n_measures){
    ...
    tree_growth_mean[t] <-  ((tree_effect[tree_index[t]] + ndep_baseline[t]*p4 + ndep_delta[t]*p5 ) 
    * tree_agb_obs[t] ^ p2) 
        * exp(-ba_gt[t]*p3)  
