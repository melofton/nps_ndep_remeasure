...
check <- focal_df3 %>%
  select(plot_ID, level1_ecoregion) %>%
  distinct(.) %>%
  group_by(level1_ecoregion) %>%
  summarize(count = n_distinct(plot_ID))
...
#write.csv(df, "./data/processed_data_test.csv", row.names = FALSE)

#df <- read_csv("./data/processed_data.csv")

total_species <- length(unique(df$common_name))

sim <- "ortho_log_interaction_adj_priors"
print(sim)
...
source("./modeling_code/ortho_log_interaction_adj_priors.R")
